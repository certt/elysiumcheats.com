...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            g.OverlayDrawList.AddImage(tex_id, pos,             pos + size,             cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(245,74,74,255)); // White fill
...
{
	ImGuiWindow* window = GetCurrentWindow();
	ImGuiWindowFlags flags = ImGuiWindowFlags_GroupBox | ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow;
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->WindowPadding = (!(flags & ImGuiWindowFlags_GroupBox) && (flags & ImGuiWindowFlags_ChildWindow) && !(flags & (ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_ShowBorders | ImGuiWindowFlags_ComboBox | ImGuiWindowFlags_Popup))) ? ImVec2(0,0) : style.WindowPadding;
        ...
        else
        {
            ...
            window->BorderSize = (flags & ImGuiWindowFlags_ShowBorders || flags & ImGuiWindowFlags_GroupBox) ? 1.0f : 0.0f;
			...
			{
				window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetColorU32(ImGuiCol_MainColor), 0.0f, -1, 6.0f);
				window->DrawList->AddRectFilled(window->Pos, ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 22.0f), GetColorU32(ImGuiCol_MainColor));
			}*/

            // Borders
            if (flags & ImGuiWindowFlags_ShowBorders && !(flags & ImGuiWindowFlags_GroupBox))
            {
			...
			else if (flags & ImGuiWindowFlags_GroupBox) {
				auto text_size = CalcTextSize(name, NULL, true);

				if (text_size.x > 1.0f) {
					window->DrawList->AddLine(window->Pos, window->Pos + ImVec2{ (GetWindowContentRegionMax().x / 2) - (text_size.x / 2) - window->WindowPadding.x * 0.5f, 0.0f }, GetColorU32(ImGuiCol_Border));
					window->DrawList->AddLine(window->Pos + ImVec2{ window->WindowPadding.x * 0.5f + (GetWindowContentRegionMax().x / 2) + (text_size.x / 2), 0.0f }, window->Pos + ImVec2{ window->Size.x, 0.0f }, GetColorU32(ImGuiCol_Border));

					window->DrawList->AddLine(window->Pos, window->Pos + ImVec2{ 0.0f, window->Size.y }, GetColorU32(ImGuiCol_Border));

					window->DrawList->AddLine(window->Pos + ImVec2{ window->Size.x, 0.0f }, window->Pos + window->Size + ImVec2{ 0.0f, 0.5f }, GetColorU32(ImGuiCol_Border));

					window->DrawList->AddLine(window->Pos + ImVec2{ 0.0f, window->Size.y }, window->Pos + window->Size + ImVec2{ 0.5f, 0.0f }, GetColorU32(ImGuiCol_Border));
					window->DrawList->PushClipRectFullScreen();
					RenderText(window->Pos + ImVec2{ (GetWindowContentRegionMax().x / 2) - (text_size.x / 2), -text_size.y / 2 }, name, NULL, true);
					window->DrawList->PopClipRect();
				}
				else {
					window->DrawList->AddRect(window->Pos + ImVec2(1, 1), window->Pos + window->Size + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow), window_rounding);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_MainColor : hovered ? ImGuiCol_MainColor : ImGuiCol_MainColor);
...
bool ImGui::Tab(const char* label, const ImVec2& size_arg, bool Enabled, int Number)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	{
		if (Number == 5)
			window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x - 1.50f, bb.Max.y), GetColorU32(ImGuiCol_MainColor));
		else
			window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(ImGuiCol_MainColor));

		//	window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x, bb.Min.y + 25.0f), ImGui::GetColorU32(ImGuiCol_MainColor), 3.0f);
		ImGui::GetStyle().Colors[ImGuiCol_Text] = ImVec4(1.00f, 1.00f, 1.00f, 0.80f);
		//window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y) + ImVec2(1,1), ImVec2(bb.Min.x, bb.Max.y) + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow));
		//window->DrawList->AddLine(ImVec2(bb.Max.x, bb.Min.y) + ImVec2(1, 1), ImVec2(bb.Max.x, bb.Max.y) + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow));

		if (Number == 5)
			window->DrawList->AddLine(ImVec2(bb.Min.x - 2.0f, bb.Min.y), ImVec2(bb.Max.x - 1.0f, bb.Min.y), GetColorU32(ImGuiCol_Border));
		else
			window->DrawList->AddLine(ImVec2(bb.Min.x - 2.0f, bb.Min.y), ImVec2(bb.Max.x + 2.0f, bb.Min.y), GetColorU32(ImGuiCol_Border));

		if (Number != 0)
			window->DrawList->AddLine(ImVec2(bb.Min.x - 1.0f, bb.Min.y), ImVec2(bb.Min.x - 1.0f, bb.Max.y), GetColorU32(ImGuiCol_Border));
		if (Number != 5)
			window->DrawList->AddLine(ImVec2(bb.Max.x - 1.0f, bb.Min.y), ImVec2(bb.Max.x - 1.0f, bb.Max.y), GetColorU32(ImGuiCol_Border));
	}
	else
	{
		if (Number == 5)
			window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x - 1.50f, bb.Max.y), Color(191, 191, 193, 255).GetU32());
		else
			window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), Color(191, 191, 193, 255).GetU32());

		if (!hovered)
			ImGui::GetStyle().Colors[ImGuiCol_Text] = ImVec4(0.12f, 0.11f, 0.11f, 0.50f);
		else
			ImGui::GetStyle().Colors[ImGuiCol_Text] = ImVec4(0.12f, 0.11f, 0.11f, 0.80f);
		// vertical lines +0.2f
		//window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y) + ImVec2(1, 1), ImVec2(bb.Max.x, bb.Min.y) + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow));
		if (Number == 5)
			window->DrawList->AddLine(ImVec2(bb.Min.x - 2.0f, bb.Min.y), ImVec2(bb.Max.x - 1.0f, bb.Min.y), GetColorU32(ImGuiCol_Border));
		else
			window->DrawList->AddLine(ImVec2(bb.Min.x - 2.0f, bb.Min.y), ImVec2(bb.Max.x + 2.0f, bb.Min.y), GetColorU32(ImGuiCol_Border));
	...
	if (Number == 0)
		window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y - 25.0f), ImVec2(bb.Min.x, bb.Max.y), GetColorU32(ImGuiCol_Border));

	if (Number == 5)
		window->DrawList->AddLine(ImVec2(bb.Max.x - 2.0f, bb.Min.y - 25.0f), ImVec2(bb.Max.x - 2.0f, bb.Max.y), GetColorU32(ImGuiCol_Border));

	RenderTextClipped(bb.Min + style.FramePadding, ImVec2(bb.Max.x - style.FramePadding.x, bb.Max.y - style.FramePadding.y), label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(2, 2), grab_bb.Max - ImVec2(0, 2), ImGui::GetColorU32(ImGuiCol_MainColor), style.GrabRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		...
		RenderCheckMark(check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_MainColor), check_bb.GetWidth() - pad * 2.0f);
	}
	else
	{
		const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	const ImVec2 label_size2 = CalcTextSize(buf_display, NULL, true);
	//RenderTextClipped(frame_bb.Max - ImVec2(label_size2.x, 0.0f), frame_bb.Max - style.FramePadding, buf_display, NULL, NULL);

	if (label_size2.x > 0)
		RenderText(ImVec2(frame_bb.Max.x - label_size2.x, frame_bb.Min.y + style.FramePadding.y), buf_display);
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w - 2.0f, label_size.y + style.FramePadding.y*2.0f));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, &id))
		return false;
	...
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);

	window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size + 2.5f, frame_bb.Min.y + 3.0f), ImVec2(frame_bb.Max.x - 2.5f, frame_bb.Min.y + 7.0f), GetColorU32(ImGuiCol_MainColor), style.FrameRounding);
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size + 2.5f, frame_bb.Min.y + 3.0f), ImVec2(frame_bb.Max.x - 2.5f, frame_bb.Min.y + 7.0f), GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);

	window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size + 2.5f, frame_bb.Min.y + 8.0f), ImVec2(frame_bb.Max.x - 2.5f, frame_bb.Min.y + 12.0f), GetColorU32(ImGuiCol_MainColor), style.FrameRounding);
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size + 2.5f, frame_bb.Min.y + 8.0f), ImVec2(frame_bb.Max.x - 2.5f, frame_bb.Min.y + 12.0f), GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);

	window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size + 2.5f, frame_bb.Min.y + 13.0f), ImVec2(frame_bb.Max.x - 2.0f, frame_bb.Min.y + 17.0f), GetColorU32(ImGuiCol_MainColor), style.FrameRounding);
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size + 2.5f, frame_bb.Min.y + 13.0f), ImVec2(frame_bb.Max.x - 2.5f, frame_bb.Min.y + 17.0f), GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);

	/*RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Min.x + arrow_size, frame_bb.Max.y), GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);

	window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 2.0f, frame_bb.Min.y + 3.0f), ImVec2(frame_bb.Min.x + arrow_size - 2.0f, frame_bb.Min.y + 7.0f), GetColorU32(ImGuiCol_MainColor), 15.0f);
	RenderFrame(ImVec2(frame_bb.Min.x + 2.0f, frame_bb.Min.y + 3.0f), ImVec2(frame_bb.Min.x + arrow_size - 2.0f, frame_bb.Min.y + 7.0f), GetColorU32(ImGuiCol_FrameBg), true, 15.0f);

	window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 2.0f, frame_bb.Min.y + 8.0f), ImVec2(frame_bb.Min.x + arrow_size - 2.0f, frame_bb.Min.y + 12.0f), GetColorU32(ImGuiCol_MainColor), 15.0f);
	RenderFrame(ImVec2(frame_bb.Min.x + 2.0f, frame_bb.Min.y + 8.0f), ImVec2(frame_bb.Min.x + arrow_size - 2.0f, frame_bb.Min.y + 12.0f), GetColorU32(ImGuiCol_FrameBg), true, 15.0f);

	window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 2.0f, frame_bb.Min.y + 13.0f), ImVec2(frame_bb.Min.x + arrow_size - 2.0f, frame_bb.Min.y + 17.0f), GetColorU32(ImGuiCol_MainColor), 15.0f);
	RenderFrame(ImVec2(frame_bb.Min.x + 2.0f, frame_bb.Min.y + 13.0f), ImVec2(frame_bb.Min.x + arrow_size - 2.0f, frame_bb.Min.y + 17.0f), GetColorU32(ImGuiCol_FrameBg), true, 15.0f);
	*/
	if (*current_item >= 0 && *current_item < items_count)
	{
		const char* item_text;
		if (items_getter(data, *current_item, &item_text))
			RenderTextClipped(frame_bb.Min + ImVec2(3.0f /*+ arrow_size*/, 0.0f), value_bb.Max, item_text, NULL, NULL, ImVec2(0.0f, 0.0f));
	}
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (selected)
        RenderCheckMark(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f, 0.0f), GetColorU32(enabled ? ImGuiCol_Text : ImGuiCol_TextDisabled), g.FontSize  * 0.866f);
